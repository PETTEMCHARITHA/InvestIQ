...
// import BackgroundImage from './Loginpage.png';
...
//   // Verify OTP and save user details to the database
//   const handleVerifyOtp = async (e) => {
//     e.preventDefault();
//     try {
//       const response = await axios.post('http://localhost:5000/api/otp/verify-otp', { email, otp });
...
//         const saveResponse = await axios.post('http://localhost:5000/api/users/signup', {
//           firstName,
//           lastName,
//           email,
//           password,
...
//           toggleAuthView(); // Switch to login form
//         } else {
...
//       alert('Failed to verify OTP or save user details. Please try again.');
//     }
//   };
...
//   // Fetch Google profile after Google login
//   useEffect(() => {
//     if (googleUser) {
//       const fetchGoogleProfile = async () => {
//         try {
...
import { useGoogleLogin } from '@react-oauth/google';
import '../Components/AuthToggle.css';
import BackgroundImage from './Loginpage.png'; // Import the background image
...
const AuthToggle = () => {
  ...
  const handleSendOtp = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/otp/send-otp', { email });
      if (response.data.success) {
        displayMessage('success', 'OTP sent to your email.');
        setIsOtpSent(true);
      } else {
      ...
      console.error('Error sending OTP:', error);
      displayMessage('error', 'Failed to send OTP. Please try again.');
    }
  };
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (saveResponse.status === 200) {
          displayMessage('success', 'Account created successfully! You can now log in.');
          toggleAuthView(); // Switch to login form
        } else {
          displayMessage('error', 'Failed to create account. Please try again.');
        }
      } else {
        displayMessage('error', response.data.message);
      }
      ...
      console.error('Error verifying OTP or saving user:', error);
      displayMessage('error', 'Failed to verify OTP or save user details. Please try again.');
    }
  };
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      console.error('Error during login:', error);
      displayMessage('error', 'Failed to log in. Please check your credentials and try again.');
    }
  };
    ...
    if (googleUser) {
      const fetchGoogleProfile = async () => {
        try {
          ...
          console.error('Google profile fetch failed:', error);
          displayMessage('error', 'Failed to fetch Google profile.');
        }
      };
